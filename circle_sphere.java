...
public class circle_sphere extends ActionBarActivity {

    TextView t1, t2, t3, t4;
    double area, circumference, surfaceArea, volume, keyValue;
    Button returnButton;
    final String APPTAG = "COM337";
    String dataKey = "circleSphereCalcs";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_circle_sphere);
        ...
        String newData = passedData.getString("circle key data");
        Log.v(APPTAG, ":circle_sphere.class: " + "data passed");

        //setting up TextViews and buttons
        t1 = (TextView) findViewById(R.id.circle_answer11);
        t2 = (TextView) findViewById(R.id.circle_answer21);
        t3 = (TextView) findViewById(R.id.sphere_answer11);
        t4 = (TextView) findViewById(R.id.sphere_answer21);
        Log.v(APPTAG, ":circle_sphere.class: " + "views setup successfully");

        //preforming the calculations below
        keyValue = Double.parseDouble(newData);
        area = Math.pow(keyValue, 2) * Math.PI;
        circumference = 2 * keyValue * Math.PI;
        surfaceArea = 4 * Math.PI * Math.pow(keyValue, 2);
        volume = (4 * Math.PI * Math.pow(keyValue, 3))/3;
        Log.v(APPTAG, ":circle_sphere.class: " + "calculations completed successfully");
        ...
        Log.v(APPTAG, ":circle_sphere.class: " + "data set to views");
    ...
    protected void saveData(Double keyValue, Double area, Double circumference, Double surfaceArea, Double volume){
        ...
        previousCalculations = previousCalculations + "Radius = " + keyValue + "\nThe circle's area will be " + String.format("%.2f",area) + " and the circumference will be " + String.format("%.2f",circumference) + ". "
                                + "The sphere's surfaceArea will be " + String.format("%.2f",surfaceArea) + " and the volume will be " + String.format("%.2f",volume) + ".\n";

        editor.putString(dataKey, previousCalculations);
        editor.commit();
        Log.v(APPTAG, ":circle_sphere: editor committed");
    }
    ...
    protected void onStart() {
        super.onStart();
        Log.v(APPTAG, "circle/sphere: onStart() called");
    }
    @Override
    protected void onResume() {
        super.onResume();
        updateBackgroundColour();
        Log.v(APPTAG, "circle/sphere: onResume() called");
    }
    @Override
    protected void onPause() { super.onPause();
        Log.v(APPTAG, "circle/sphere: onPause() called"); }
    @Override
    protected void onStop() { super.onStop();
        Log.v(APPTAG, "circle/sphere: onStop() called"); }
    @Override
    protected void onDestroy() {
        super.onDestroy();
        Log.v(APPTAG, "circle/sphere: onDestroy() called");
    }
}
